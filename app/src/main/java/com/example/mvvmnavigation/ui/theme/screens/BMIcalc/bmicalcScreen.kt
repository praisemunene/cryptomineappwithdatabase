...
fun getContentFromReadPHP(context: Context,price:String,amountaffordable:String) {
    val queue = Volley.newRequestQueue(context)
    val url = "http://192.168.1.3/restapi/postad.php"
//    val url = "https://cryptomine.rf.gd/reload.php"


    val stringRequest = @SuppressLint("SuspiciousIndentation")
    object : StringRequest(
        Method.GET, "$url?amount=$amountaffordable&price=$price",
        Response.Listener<String> { response ->
            val message = "Data posted successfully"
...
val context = LocalContext.current
                    Button(
                        onClick = {
                            getContentFromReadPHP(context = context,price,amountaffordable)
                        },
                        modifier = Modifier
                            .fillMaxWidth(0.9f)
                            .height(45.dp),
